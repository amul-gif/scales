...
void setup() {
 size(500, 500);  //canvas size
  ...
  //Defining colors
   b1 = color(255);
  b2 = color(0);
  c1 = color(204, 102, 0); //orange in RGB
  c2 = color(81, 110, 87); //green in RGB
...
// Background gradient
  ...
  // Foreground
  setGradient(0, 0, 500, 500, c1, c2, Y_AXIS); //impacts the size of gradient rectangle across the entire canvas, coordinates (0,0,500,500)
  setGradient(0, 0, 500, 500, c2, c1, X_AXIS); //impacts the color of gradient actually showing up on the canvas
...
void mainScale(int x, int y) { //x and y variables for main scale
  noFill();
  strokeWeight(2); // increases stroke weight
...
//code from Processing reference library -- modified to fit functionality of this program; changing colors and size of gradient
void setGradient(int x, int y, float w, float h, color c1, color c2, int axis ) { //function for gradient
  ...
  if (axis == X_AXIS) {  // Left to right gradient
    for (int i = x; i <= x+w; i++) { //for loop to create gradient
      ...
      stroke(c);//if turned off makes entire canvas black
      line(i, y, i, y+h);
    }
  }
}
